...
async def countdown(event, target_datetime):
    ...
    for remaining in range(time_difference.seconds, -1, -1):
        ...
        countdown_format = (
            ...
            f"{' '.join(random.choices(emojis))} **HOURS LEFT:** {hours} HOURS {' '.join(random.choices(emojis))}\n\n\n\n"
            f"{' '.join(random.choices(emojis))} **MINUTES LEFT:** {minutes} MINUTES {' '.join(random.choices(emojis))}\n\n\n\n"
            f"{' '.join(random.choices(emojis))} **SECONDS LEFT:** {seconds} SECONDS {' '.join(random.choices(emojis))}\n\n\n\n\n"
            f"{random.choice(exit_messages)}\n"
            f"@Notfound989"
        )

        await message.edit(countdown_format)
        await asyncio.sleep(1)

    await message.edit("🎉 **Countdown Finished!**")
